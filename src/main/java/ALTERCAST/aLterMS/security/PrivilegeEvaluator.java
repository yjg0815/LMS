...
public class PrivilegeEvaluator {
    ...
    private void printPrivileges(Authentication authentication) {
        System.out.print("PermissionEvaluator: account id '" + authentication.getName() + "' with role [ ");
     ...
     */
    public boolean hasPrivilege(Long secId, Privilege privilege) {
        // 권한 출력
        ...
        for (GrantedAuthority grantedAuth : authentication.getAuthorities()) {
            if (grantedAuth.getAuthority().endsWith(String.valueOf(secId)) && grantedAuth.getAuthority().contains(privilege.toString())) {
                return true;
            }
        }
        return false;
